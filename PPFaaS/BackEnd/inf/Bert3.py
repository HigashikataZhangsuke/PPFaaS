...
class BertModelPart1(torch.nn.Module):
    def __init__(self, model):
        ...
        self.encoder = torch.nn.ModuleList([model.encoder.layer[i] for i in range(16)])  # 前12层
...
class BertModelPart2(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        self.encoder = torch.nn.ModuleList([model.encoder.layer[i] for i in range(16, 32)])  # 后12层
...
class BertModelPart41(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        self.embeddings = model.embeddings
        self.encoder = torch.nn.ModuleList([model.encoder.layer[i] for i in range(8)])  # 前12层
...
class BertModelPart42(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        #self.embeddings = model.embeddings
        self.encoder = torch.nn.ModuleList([model.encoder.layer[i] for i in range(8,16)])  # 前12层
...
class BertModelPart43(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        #self.embeddings = model.embeddings
        self.encoder = torch.nn.ModuleList([model.encoder.layer[i] for i in range(16, 24)])  # 前12层
...
class BertModelPart44(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        self.encoder = torch.nn.ModuleList([model.encoder.layer[i] for i in range(24, 32)])  # 后12层
        self.pooler = model.pooler
    ...
    while True:
        ...
        if not queue.empty():
            ...
            if data == 'None':
                ...
                msk_shape = torch.tensor(attention_mask.shape, dtype=torch.int64).to('cuda:0')
                dist.send(tensor=msk_shape, dst=1)
                dist.send(tensor=attention_mask, dst=1)
                outputshape = torch.tensor(output.shape, dtype=torch.int64).to('cuda:0')
    ...
    while True:
        ...
        if not informqueue.empty():
            ...
            #
                ...
                if not batchqueue.empty():
                    batch = batchqueue.get()
                mask_shape = torch.empty(2, dtype=torch.int64).to('cuda:1')
                dist.recv(tensor=mask_shape, src=0)
                msktemp = torch.empty(tuple(mask_shape.tolist())).to('cuda:1')
                dist.recv(tensor=msktemp, src=0)
                inter_shape = torch.empty(3, dtype=torch.int64).to('cuda:1')
                dist.recv(tensor=inter_shape, src=0)
                data1 = torch.empty(tuple(inter_shape.tolist())).to('cuda:1')
        ...
        if not informqueue.empty():
            ...
            if chra == 'stop':
                ...
                if not batchqueue.empty():
                    batch = batchqueue.get()
                informqueue2.put('send')
                mask_shape = torch.empty(2, dtype=torch.int64).to('cuda:2')
                dist.recv(tensor=mask_shape, src=1)
                msktemp = torch.empty(tuple(mask_shape.tolist())).to('cuda:2')
                dist.recv(tensor=msktemp, src=1)
                inter_shape = torch.empty(3, dtype=torch.int64).to('cuda:2')
                dist.recv(tensor=inter_shape, src=1)
                data1 = torch.empty(tuple(inter_shape.tolist())).to('cuda:2')
    ...
    while True:
        ...
        if not informqueue.empty():
            ...
            if chra == 'stop':
                ...
                mask_shape = torch.empty(2, dtype=torch.int64).to('cuda:3')
                dist.recv(tensor=mask_shape, src=2)
                msktemp = torch.empty(tuple(mask_shape.tolist())).to('cuda:3')
                dist.recv(tensor=msktemp, src=2)
                inter_shape = torch.empty(3, dtype=torch.int64).to('cuda:3')
                dist.recv(tensor=inter_shape, src=2)
                data1 = torch.empty(tuple(inter_shape.tolist())).to('cuda:3')
...
def mainlogicbert3(redisqueuename,MyRedisQueue,paraqueue):
    ...
    bertmodel = BertModel.from_pretrained('/home/ubuntu/modelstorage/model34B')
    curr_p = 4
