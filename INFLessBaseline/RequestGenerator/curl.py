...
broker_url2 = "http://broker-ingress.knative-eventing.svc.cluster.local/default/defaulttwo"
broker_url3 = "http://broker-ingress.knative-eventing.svc.cluster.local/default/defaultthree"
broker_url4 = "http://broker-ingress.knative-eventing.svc.cluster.local/default/defaultfour"
broker_url5 = "http://broker-ingress.knative-eventing.svc.cluster.local/default/defaultfive"
broker_url6 = "http://broker-ingress.knative-eventing.svc.cluster.local/default/defaultsix"
...
async def run_async_tasks2(text):
    input = text
    random_SLO = 1.5*0.25
    json_data = {
        "Bert2": input,
    ...
    interval =1.5 #Lambda = 5
    duration = 120  # 测试持续时间，例如60秒
    requests_sent = await continuous_request(broker_url2, json_data, headers, interval, duration)

def run_in_process2(tensor):
    asyncio.run(run_async_tasks2(tensor))

async def run_async_tasks3(text):
    input = text
    random_SLO = 1.5*1
    json_data = {
        "Bert3": input,
    ...
    interval =5 #Lambda = 5
    duration = 120  # 测试持续时间，例如60秒
    requests_sent = await continuous_request(broker_url3, json_data, headers, interval, duration)

def run_in_process3(tensor):
    asyncio.run(run_async_tasks3(tensor))

async def run_async_tasks4(text):
    input = text
    random_SLO = 1.5*0.25
    json_data = {
        "Bert4": input,
    ...
    interval =1.5 #Lambda = 5
    duration = 120  # 测试持续时间，例如60秒
    requests_sent = await continuous_request(broker_url4, json_data, headers, interval, duration)

def run_in_process4(tensor):
    asyncio.run(run_async_tasks4(tensor))

async def run_async_tasks5(text):
    input = text
    random_SLO = 1.5*1
    json_data = {
        "Bert5": input,
    ...
    interval =5 #Lambda = 5
    duration = 120  # 测试持续时间，例如60秒
    requests_sent = await continuous_request(broker_url5, json_data, headers, interval, duration)

def run_in_process5(tensor):
    asyncio.run(run_async_tasks5(tensor))

async def run_async_tasks6(text):
    input = text
    random_SLO = 1.5*0.25
    json_data = {
        "Bert6": input,
    ...
    interval =1.5 #Lambda = 5
    duration = 120  # 测试持续时间，例如60秒
    requests_sent = await continuous_request(broker_url6, json_data, headers, interval, duration)

def run_in_process6(tensor):
    asyncio.run(run_async_tasks6(tensor))
...
if __name__ == '__main__':
    ...
    p1 = Process(target=run_in_process2, args=(tensors,))
    p2 = Process(target=run_in_process3, args=(tensors,))
    p3 = Process(target=run_in_process4, args=(tensors,))
    p4 = Process(target=run_in_process5, args=(tensors,))
    p5 = Process(target=run_in_process6, args=(tensors,))
    p0.start()
    p1.start()
    p2.start()
    p3.start()
...
# lamda = 1.5  # 单位时间内事件发生的平均次数
# duration = 10000  # 模拟的总时间长度
# events = poisson_process(lamda, duration)
# #print("事件发生的时间点：", events)
# assumerate = getrate(events)
# print(assumerate)
#Testing Gaussian
# import numpy as np
